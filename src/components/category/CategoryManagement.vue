          ...
          <table>
            ...
            <tbody v-if="!loading && visibleRows.length">
              <tr v-for="row in visibleRows" :key="row.id">
                ...
                <td><div class="category-name" :style="{ paddingLeft: `${row.depth * 25}px` }"><button v-if="row.hasChildren" class="expand" :class="{ open: expanded.has(row.id) }" @click="toggleExpand(row.id)">›</button><span v-else class="expand-spacer"></span><AppIcon name="folder" :size="17" class="folder-icon" /><strong>{{ row.name }}</strong></div></td>
                ...
                <td>{{ formatDate(row.updatedAt) }}</td><td><div class="row-actions"><button title="编辑" @click="openEdit(row)">✎</button><button class="delete-btn" title="删除" @click="removeOne(row)"><AppIcon name="trash" :size="16" /></button></div></td>
              </tr>
            </tbody>
          </table>
